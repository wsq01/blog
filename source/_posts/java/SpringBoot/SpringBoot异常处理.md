...
换句话说，当使用浏览器访问出现异常时，会进入`BasicErrorController`控制器中的`errorHtml()`方法进行处理，当使用安卓、IOS、Postman 等机器客户端访问出现异常时，就进入`error()`方法处理。
...
1）在`net.biancheng.net.exception`包内，创建一个名为`UserNotExistException`的异常类。
```java
...
```
2）在`IndexController`添加以下方法，触发`UserNotExistException`异常。
```java
@Controller
...
```
3）在`net.biancheng.www.controller`中，创建一个名为`MyExceptionHandler`异常处理类。
```java
...
```
### 2. 自定义错误属性处理工具
1）在`net.biancheng.www.componet`包内，创建一个错误属性处理工具类`MyErrorAttributes`（继承`DefaultErrorAttributes`），通过该类我们便可以添加自定义的错误数据。
```java
...
```
2）在`templates/error`目录下，创建动态错误页面`5xx.html`。
```html
<!DOCTYPE html>
<html lang="en" xmlns:th="http://www.thymeleaf.org">
<head>
...
```
3）启动 Spring Boot，访问`http://localhost:8080/testException?user=user`。

...
| aggressiveLazyLoading            | 开启时，任一方法的调用都会加载该对象的所有延迟加载属性。<br>否则，每个延迟加载属性会按需加载(参考 lazyLoadTriggerMethods)。 | true, false | false(3.4.1版本及之前默认为true) |
...
| defaultResultSetType             | 指定语句默认的滚动策略。（新增于 3.5.2） | ORWARD_ONLY,<br>SCROLL_SENSITIVE,<br>SCROLL_INSENSITIVE,<br> DEFAULT（等同于未设置） | 未设置 (null) |

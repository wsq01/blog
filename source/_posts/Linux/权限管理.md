...
`chown`命令，`change owner`的缩写，主要用于修改文件或目录的所有者，也可以修改文件或目录的所属组。
...
```
# 权限位
最常见的文件权限有 3 种，即对文件的读（用`r`表示）、写（用`w`表示）和执行（用`x`表示，针对可执行文件或目录）权限。在 Linux 系统中，每个文件都明确规定了不同身份用户的访问权限，通过`ls`命令即可看到。
```bash

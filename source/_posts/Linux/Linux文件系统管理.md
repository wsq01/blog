...
在 CentOS 6.3 系统中，默认的文件系统是 Ext4。
...
上图中，文件系统先格式化出`inode`和`block`块，假设某文件的权限和属性信息存放到`inode 4`号位置，这个`inode`记录了实际存储文件数据的`block`号有 4 个，分别为 2、7、13、15，由此，操作系统就能快速地找到文件数据的存储位置。

...
| TINYTEXT | 非常小的非二进制字符串 | L+1 字节，在此，L<2<sup>8</sup> |
| TEXT | 小的非二进制字符串 | L+2 字节，在此，L<2<sup>16</sup> |
| MEDIUMTEXT | 中等大小的非二进制字符串 | L+3 字节，在此，L<2<sup>24</sup> |
...
* `TINYTEXT`表示长度为 255（2<sup>8</sup>-1）字符的`TEXT`列。
* `TEXT`表示长度为 65535（2<sup>16</sup>-1）字符的`TEXT`列。
* `MEDIUMTEXT`表示长度为 16777215（2<sup>24</sup>-1）字符的`TEXT`列。
* `LONGTEXT`表示长度为 4294967295 或 4GB（2<sup>32</sup>-1）字符的`TEXT`列。

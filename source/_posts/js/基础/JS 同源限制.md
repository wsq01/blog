...
另外，通过 JavaScript 脚本可以拿到其他窗口的`window`对象。如果是非同源的网页，目前允许一个窗口可以接触其他网页的`window`对象的九个属性和四个方法：`window.closed`、`window.frames`、`window.length`、`window.location`、`window.opener`、`window.parent`、`window.self`、`window.top`、`window.window`、`window.blur()`、`window.close()`、`window.focus()`、`window.postMessage()`。

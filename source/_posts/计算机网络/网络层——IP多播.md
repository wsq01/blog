...
# IP 多播的基本概念
IP 多播 (`multicast`，以前曾译为组播) 目的：更好地支持一对多通信。
...
在多播数据报的目的地址写入的是多播组的标识符。多播组的标识符就是 IP 地址中的 D 类地址（多播地址）。每一个 D 类地址标志一个多播组。
...
# 在局域网上进行硬件多播
互联网号码指派管理局 IANA 拥有的以太网地址块的高 24 位为`00-00-5E`。因此 TCP/IP 协议使用的以太网地址块的范围是从`00-00-5E-00-00-00`到`00-00-5E-FF-FF-FF`。

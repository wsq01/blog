...
下面是一个具体的服务发布配置文件`user-last-status.xml`，它定义了要发布的接口`userLastStatusLocalService`，对外暴露的协议是 Motan 协议，端口是 8882。并且针对两个方法`getLastStatusId`和`getLastStatusIds`，通过`requestTimeout="300"`单独定义了超时时间是 300ms，通过`retries="0"`单独定义了调用失败后重试次数为 0，也就是不重试。
```xml

...
{% asset_img img1.png MySQL 的逻辑架构图 %}
...
连接完成后，如果你没有后续的动作，这个连接就处于空闲状态，你可以在`show processlist`命令中看到它。下面这个图是`show processlist`的结果，其中的`Command`列显示为`Sleep`的这一行，就表示现在系统里面有一个空闲连接。
